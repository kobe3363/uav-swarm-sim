...
- [ ] **C2:** STUDY-01 re-run with the new RTH. **E2 gate satisfied** (`--jobs` delivered + byte-identity proven for `run_spare_sizing --demand-mode`); ~30 h serial → core-bound with `--jobs auto` on the run box.
...
- [x] **E3:** Visibility-graph caching for `route_transit`. **DELIVERED (patch, branch `e3-visibility-graph-cache`).** The endpoint-independent O(V²) obstacle-vertex visibility result is memoised on a per-replication engine dict, keyed by `(sha1(obs.wkb), operating_area, margin_m)`, and spliced into each `(a,b)` query byte-identically (cache consulted purely by coordinate-key pair, never by node index → the a/b-on-vertex dedup shift is irrelevant; forced-collision seam test proves it). Only the True (visible) edge set is stored → O(visible), not O(V²). **Cross-commit golden vs pre-E3 `0ed3dad`** (dense study01_demand rep 1, `transit_free_space` ON): cached ≡ uncached bit-for-bit. Measured on that rep: **V=1349, 168,540 visible edges cached (≈ tens of MB/worker), 5.88× speedup (898→153 s)**; also shortens the `test_transit_livelock` B1 leg the same way. Per-worker under `--jobs` (spawn), E2 determinism gate still green. Suite 474 → 482 green (+1 skip; note: the previously documented 473 baseline measured 474 pre-E3 on this box). **Unblocks C3.**
