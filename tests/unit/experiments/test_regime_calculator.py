...
def _planning_bits(config_path):
    cfg = load_config(config_path)
    spec = build_spec(cfg)
    return spec, EnergyModel(spec), make_motion_model(spec)


def test_coverage_energy_breakdown_is_consistent(config_path):
    spec, em, motion = _planning_bits(config_path)
...
def test_camera_payload_only_adds_energy_on_strips(config_path):
    spec, em, motion = _planning_bits(config_path)
    poly = normalize_to_area(shape_builders(128)["square"], 1_000_000.0)
...
def test_usable_fractions_ordered(config_path):
    cfg = load_config(config_path)
...
def test_per_zone_energy_is_assignment_aware(config_path):
...
def test_analytical_coverage_matches_engine_swap_free(config_path):
